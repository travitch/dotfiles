...
(use-package magit
  :ensure t
  :after transient
  :defines (magit-auto-revert-mode magit-auto-revert-immediately magit-diff-refine-hunk git-commit-major-mode git-commit-summary-max-length git-commit-turn-on-flyspell)
  :init
...
If I let Windows handle DPI everything looks blurry."
  ...
  (let* ((attrs (frame-monitor-attributes)) ;; gets attribs for current frame
    ...
    (cond ((eq width-px 1920) (setq size 10))
          ...
          (t (setq size 16)))
    (set-frame-font (font-spec :family "Monaspace Neon NF" :size size))))
